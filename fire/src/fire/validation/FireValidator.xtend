...
class FireValidator extends AbstractFireValidator {
	...
	def void typeCheckMultiplicativeExpression(MultiplicativeExpression expression) {
		...
		if (leftType != null && rightType != null) {
			val operatorKeyword = expression.node.leafNodes.findFirst[grammarElement instanceof EnumLiteralDeclaration]
			val message = '''«operatorKeyword.text» operator cannot be applied to types «leftType» and «rightType»'''
